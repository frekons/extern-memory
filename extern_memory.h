...
public:
	extern_memory(uint32_t process_id, DWORD desired_access = PROCESS_ALL_ACCESS);

	extern_memory(std::string process_name, DWORD desired_access = PROCESS_ALL_ACCESS);

	virtual ~extern_memory();
...
template<class T>
inline T extern_memory::read(uintptr_t address)
{
	T value;
...
template<class T>
inline bool extern_memory::write(uintptr_t address, T value)
{
...
template<class T>
inline T extern_memory::operator()(uintptr_t address)
{
	return read<T>(address);
}

template<class T>
inline T extern_memory::operator()(std::string module_name, uintptr_t address, T type)
{
